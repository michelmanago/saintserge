...
function removeClassEffect(eltClassName) {
    ...
    for (const eltClass of eltClasses) {
        const match = eltClass === 'w-1/3' || eltClass === 'w-2/3';
        if (match) break;
        index++;
    }
    eltClasses.splice(index, 1);
...
const serialize = node => {
    ...
    if (!widthFind && classEffect) {
        if (classEffect === 'page__imgfloatleft' || classEffect === 'page__imgfloatright') classNameArray.push('w-1/3');
        if (classEffect === 'page__imgaligncenter') classNameArray.push('w-2/3');
    }
